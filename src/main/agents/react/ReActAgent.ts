...
export function createReactAgentWithSummary<
...
> {
  ...
  const summarizeConversation = async (
    ...
    const { systemMessage, keepMessages, deleteMessages, summaryMessage } =
      await checkAndSummarize(messages, model, false, {
        keepLastMessagesCount: summaryOption.keepLastMessagesCount,
      });
    const _messages = [];
    if (systemMessage) {
      _messages.push(systemMessage);
    }
    if (summaryMessage) {
  ...
  if (shouldReturnDirect.size > 0) {
    ...
    workflow.addConditionalEdges('tools', routeToolResponses, [
      'summarize_conversation',
      END,
    ]);
  } else {
    workflow.addEdge('tools', 'summarize_conversation');
